export function addRecentlyReadBooks(recentlyRead) {
  const recentlyReadSection = document.querySelector('.recently-read .books');

  if (recentlyReadSection) {
    recentlyRead.forEach(item => {
      ...
      recentlyReadSection.appendChild(figureElement)
    })
  }
}